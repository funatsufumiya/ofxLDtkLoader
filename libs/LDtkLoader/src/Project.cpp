...
void Project::loadFromFile(const std::string& filepath) {
    ...
    if (in.fail()) {
        ...
        ldtk_error("Failed to open file \"" + filepath + "\" : " + ofToString(errno));
        //ofLogError("LDtkLoader") << "Failed to open file \"" << filepath << "\" : " << ofToString(errno);
    }
...
void Project::loadFromFile(const std::string& filepath, const FileLoader& file_loader) {
    ...
    if (in.fail()) {
        ...
        ldtk_error("Failed to open file \"" + filepath + "\" using custom file loader : " + ofToString(errno));
